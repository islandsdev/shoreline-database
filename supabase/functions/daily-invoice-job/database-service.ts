...
export class DatabaseService {
  ...
  async getPaystubs(scheduleIds) {
    const { data: paystubs, error: paystubError } = await this.supabase.from("payments").select(`
    *,
  ...
  `).in("payroll_schedule_id", scheduleIds).is("invoice_id", null).order("first_name", {
  ...
  async getOneTimePayments(scheduleIds) {
    const { data: oneTimePayments, error } = await this.supabase.from("wip_one_time_payments").select(`
    *,
    employee:team_member_id (
      team_member_id,
      first_name,
  ...
  async getScheduleIds() {
    ...
    const { data: schedules, error: scheduleError } = await this.supabase.from("payroll_schedules").select("id").lte("end_date", targetDateStr1);
  ...
  async getForexRate() {
    const { data: rates, error } = await this.supabase.from("forex_rates").select("*").order("created_at", {
      ascending: false
    }).limit(1);
    if (error) {
  ...
  async getGroupInvestmentTopups() {
    const { data: topups, error } = await this.supabase.from("topups").select("*").eq("type", "Group Investment Plan").eq("status", "Completed");
    if (error) {
      throw new Error(`Error group investment plans: ${error.message}`);
    }
    return topups;
  }
  async saveInvoice(companyId, paystubIds, oneTimePaymentIds, invoice, provider = "Stripe") {
    ...
    const { data: invoiceData, error: invoiceError } = await this.supabase.from("invoices").insert({
    ...
    if (provider === "Stripe") {
      const { error: stripeError } = await this.supabase.from("stripe_invoices").insert({
      ...
      const { error: wiseError } = await this.supabase.from("wise_invoices").insert({
        invoice_id: invoiceId,
    ...
    if (paystubIds && paystubIds.length > 0) {
      const { error: paystubError } = await this.supabase.from("payments").update({
        invoice_id: invoiceId,
        status: "processing"
      }).in("id", paystubIds);
      if (paystubError) {
    ...
    if (oneTimePaymentIds && oneTimePaymentIds.length > 0) {
      const { error: oneTimeError } = await this.supabase.from("wip_one_time_payments").update({
        invoice_id: invoiceId,
        status: "processing"
      }).in("id", oneTimePaymentIds);
      if (oneTimeError) {
        throw new Error(`Error updating one-time payments: ${oneTimeError.message}`);
      }
    }
    return invoiceId;
  }
