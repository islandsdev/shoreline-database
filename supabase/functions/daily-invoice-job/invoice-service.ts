...
export async function createInvoiceAndItemsForAllCompanies(
  dbService,
  companiesData
) {
  const results = [];
  const errors = [];
  console.log(
    `Starting invoice processing with batch size: ${config.BATCH_SIZE}, max retries: ${config.MAX_RETRIES}`
  );
  ...
  for (let i = 0; i < companiesData.length; i += config.BATCH_SIZE) {
    ...
    console.log(
      `Processing batch ${batchNumber}/${totalBatches} with ${batch.length} companies`
    );
    ...
    const batchPromises = batch.map(async (company) => {
      try {
        ...
        console.error(
          `Error processing company ${company.company_name}:`,
          error
        );
        return errorInfo;
      }
    ...
    console.log(
      `Batch ${batchNumber}/${totalBatches} completed. Success: ${results.length}, Errors: ${errors.length}`
    );
    // Add a small delay between batches to prevent overwhelming external APIs
    if (i + config.BATCH_SIZE < companiesData.length) {
      if (config.ENABLE_DEBUG_LOGGING) {
        console.log(
          `Waiting ${config.INTER_BATCH_DELAY}ms before next batch...`
        );
      }
      await new Promise((resolve) =>
        setTimeout(resolve, config.INTER_BATCH_DELAY)
      );
    }
  }
  // Log summary
  console.log(
    `Processing complete. Success: ${results.length}, Errors: ${errors.length}`
  );
  return {
    success: results,
    errors: errors,
  ...
  try {
    if (company.payment_method === "Stripe") {
      if (!company.company_stripe_id) {
        throw new Error(
          `Company ${company.company_name} has Stripe payment method but no Stripe customer ID`
        );
      }
      const invoice = await createInvoiceAndItems(
        company.company_stripe_id,
        company.invoices
      );
      const invoiceId = await dbService.saveInvoice(
        company.company_id,
        company.paystubIds,
        company.oneTimePaymentIds,
        invoice,
        "Stripe"
      );
      return {
      ...
      const wiseInvoice = await createWiseInvoice(
        company.invoices,
      ...
      throw new Error(
        `Unknown payment method: ${company.payment_method} for company ${company.company_name}`
      );
    }
  } catch (error) {
