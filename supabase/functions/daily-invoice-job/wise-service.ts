...
export async function createWiseInvoice(items, companyName, companyEmail) {
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(data.error?.message || data.message || `Invoice creation failed with status ${res.status}: ${JSON.stringify(data)}`);
    }
    ...
    if (!res2.ok) {
      ...
      throw new Error(errorData.error?.message || errorData.message || `Invoice publishing failed with status ${res2.status}: ${JSON.stringify(errorData)}`);
    }
