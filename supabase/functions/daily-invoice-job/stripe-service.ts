...
export async function createInvoiceAndItems(STRIPE_API_KEY1, customerId, items) {
  try {
...
export async function createInvoice(STRIPE_API_KEY1, customerId) {
  ...
  if (!res.ok) {
    throw new Error(data.error?.message || `Invoice creation failed with status ${res.status}`);
  }
  return data;
}
export async function createInvoiceItems(STRIPE_API_KEY1, customerId, invoice, items) {
  try {
    for (const item of items){
      ...
      const data = await res.json();
      if (!res.ok) {
        throw new Error(data.error?.message || `Invoice item creation failed with status ${res.status}`);
      }
    }
  } catch (error) {
    console.error(`Error creating invoice items for invoice ${invoice.id}:`, error);
...
export async function finalizeAndPayInvoice(STRIPE_API_KEY, invoice) {
  try {
    ...
    if (!finalizeRes.ok) {
      ...
      throw new Error(finalizeData.error?.message || `Invoice finalization failed with status ${finalizeRes.status}`);
    }
    ...
    if (!payRes.ok) {
      throw new Error(payData.error?.message || `Invoice payment failed with status ${payRes.status}`);
    }
    return payData; // contains hosted_invoice_url and invoice_pdf
  } catch (error) {
